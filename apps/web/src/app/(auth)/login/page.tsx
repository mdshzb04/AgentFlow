...
import { loginSchema, type LoginFormValues } from "@/lib/schemas/security";
import { initGitHubLogin, submitLogin } from "@/lib/security";
...
function LoginContent() {
  ...
  const onSubmit = handleSubmit(async (values) => {
    setSubmitError(null);
    try {
      const { access_token } = await submitLogin(values);
      await login(access_token);
      router.replace("/dashboard");
    } catch (err) {
      setSubmitError(err instanceof ApiError ? err.message : "Login failed. Please try again.");
    }
  });
  ...
  const onGitHubLogin = async () => {
    ...
    try {
      ...
      setSubmitError(err instanceof ApiError ? err.message : "GitHub login failed.");
  ...
  return (
    <AuthFormShell
      title="Welcome back"
      description="Sign in with email or GitHub"
      footer={
        <>
    ...
    >
      {oauthError && (
        <FormAlert variant="error" message={decodeURIComponent(oauthError)} />
      )}
